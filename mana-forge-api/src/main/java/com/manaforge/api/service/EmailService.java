...
public class EmailService {
    ...
    @Async
    public void sendEmailChangeVerificationEmail(User user, String newEmail) {
        try {
            MimeMessage message = mailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(message, true, "UTF-8");
            ...
            helper.setSubject("Confirma tu nuevo correo en ManaForge");
            helper.setText(buildEmailChangeVerificationHtml(user, newEmail), true);

            mailSender.send(message);
            log.info("Email change verification email sent to {}", newEmail);
        } catch (Exception e) {
            log.error("Failed to send email change verification email to {}: {}", newEmail, e.getMessage());
        }
    }

    private String buildEmailChangeVerificationHtml(User user, String newEmail) {
        ...
        return """
              ...
              <table width="100%%" cellpadding="0" cellspacing="0" style="background:#09090b;padding:40px 0;">
                <tr><td align="center">
                  <table width="560" cellpadding="0" cellspacing="0" style="background:#18181b;border-radius:12px;overflow:hidden;border:1px solid #3f3f46;">
                    ...
                    <tr>
                      <td style="padding:32px;">
                        <h1 style="color:#fff;font-size:22px;margin:0 0 12px;">Confirma tu nuevo correo, %s</h1>
                        <p style="color:#a1a1aa;font-size:15px;line-height:1.6;margin:0 0 24px;">
                          Haz clic en el botón para confirmar tu nueva dirección de correo: %s
                        </p>
                        <a href="%s"
                           style="display:inline-block;background:#ea580c;color:#fff;text-decoration:none;padding:12px 28px;border-radius:8px;font-weight:700;font-size:15px;">
            ...
            """.formatted(user.getName(), newEmail, verifyUrl, java.time.Year.now().getValue(), frontendUrl);
    }
}
