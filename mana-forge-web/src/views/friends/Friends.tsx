...
import { Users, UserPlus, UserMinus, Check, X, Search, Clock, UserCheck, Rss } from 'lucide-react';
import {
  FriendService,
  type FriendUser,
...
export default function Friends() {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 py-10 px-4 sm:px-6 lg:px-8">
      <SEO title={t('seo.friendsTitle')} description={t('seo.friendsDescription')} />
              ...
              {isSearching ? (
                ...
                searchResults.map((user) => {
                  ...
                  return (
                    <div
                      key={user.userId}
                      className="flex flex-col sm:flex-row sm:items-center justify-between bg-zinc-800/50 rounded-xl px-4 py-3 gap-4"
                    >
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          {user.biography && (
                            <p className="text-zinc-500 text-xs truncate">{user.biography}</p>
                          )}
                        </div>
                      </div>
          ...
          {(['friends', 'requests'] as Tab[]).map((tab) => (
            <button
              ...
              className={`flex-1 py-2 px-4 rounded-lg text-sm font-medium transition-all flex items-center justify-center gap-2 ${
                activeTab === tab ? 'bg-orange-600 text-white' : 'text-zinc-400 hover:text-white'
              }`}
            >
...
function FriendsList({
  ...
  if (friends.length === 0) {
    return <EmptyState icon={<Users size={40} />} text={t('friends.noFriends')} />;
  }
  return (
    <div className="space-y-3">
      {friends.map((f) => (
        <div
          key={f.userId}
          className="bg-zinc-900 border border-zinc-800 rounded-xl px-5 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4"
        >
          <div className="flex items-center gap-3 flex-1 min-w-0">
            <img
