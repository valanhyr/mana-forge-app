...
const RadarChart: React.FC<RadarChartProps> = ({ axes, size = 260 }) => {
  ...
  const labels = axes.map((axis, i) => {
    ...
    return { x, y, label: axis.label, value: axis.value, textAnchor, dominantBaseline };
  });
      ...
      {labels.map((l, i) => (
        <g key={i}>
          ...
          <text
            ...
            dx={l.textAnchor === 'start' ? 0 : l.textAnchor === 'end' ? 0 : 0}
            textAnchor={l.textAnchor}
            dominantBaseline={
              l.dominantBaseline === 'middle' ? 'middle' : 'auto'
            }
            fontSize={11}
            fontWeight="700"
            fill="#ea580c"
