...
const ManaCost = ({ cost, symbols }: { cost?: string; symbols: Record<string, string> }) => {
  if (!cost) return null;
...
const DeckList: React.FC<DeckListProps> = ({
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (menuRef.current && !menuRef.current.contains(event.target as Node)) setActiveCard(null);
    };
  ...
  useEffect(() => {
    if (cards.length > 0) {
      ...
      setHoveredCard((prev) => prev ?? cards[0]);
    }
  }, [cards]);
      ...
      >
        ...
        {card.isGameChanger && (
          <span
            ...
            className="cursor-help inline-flex items-center justify-center bg-orange-500/20 text-orange-400 text-[10px] font-bold px-1.5 py-0.5 rounded border border-orange-500/30 no-underline shrink-0"
          >
            GC
          </span>
        )}
      </button>
      {card.isValid === false && <AlertTriangle size={11} className="text-red-500 shrink-0" />}
        ...
        <div className="flex-1 min-w-0 space-y-8">
          {groupedAndSortedCards.map(({ boardName, types }) => {
            ...
            const showSideboardWarning =
              isSideboard && maxSideboardSize !== undefined && totalBoardCards > maxSideboardSize;

            if (isCommanderFormat && isSideboard && totalBoardCards === 0) return null;
            ...
            return (
              <div key={boardName}>
                ...
                >
                  ...
                  {showPrices &&
                    (() => {
                      const boardPrice = types.reduce(
                        (sum, { cards: gc }) =>
                          sum + gc.reduce((s, c) => s + (c.price ?? 0) * c.quantity, 0),
                        0
                      );
                      return boardPrice > 0 ? (
                  ...
                  {showMainDeckWarning && (
                    <div
                      title={t('deckList.minCardsWarning', { count: minMainDeckSize || 0 })}
                  ...
                  {showSideboardWarning && (
                    <div
                      title={t('deckList.maxCardsWarning', { count: maxSideboardSize || 0 })}
                      className="text-red-500 animate-pulse"
                    >
                      <AlertTriangle size={18} />
                    </div>
              ...
              <>
                ...
                {activeCard.board !== 'maybe' && (
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      onMoveToBoard(activeCard, 'maybe');
                      setActiveCard(null);
                    }}
                    className="w-full text-left px-4 py-2 text-sm text-zinc-300 hover:bg-zinc-800 hover:text-orange-500 flex items-center gap-2"
                  >
